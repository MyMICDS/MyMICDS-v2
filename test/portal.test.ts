...
describe('Portal', () => {
	...
	describe('PUT /portal/url/classes', function () {
		...
		it('saves a classes calendar URL', async function () {
			...
			expect(userDoc)
				.to.have.property('portalURLClasses')
				.that.equals(config.portal.classesURL.replace('webcal', 'https'));
		});
	...
	describe('PUT /portal/url/calendar', function () {
		...
		it('saves a my calendar URL', async function () {
			...
			const { userDoc } = await users.get(this.db, testUser.user);
			expect(userDoc)
				.to.have.property('portalURLCalendar')
				.that.equals(config.portal.calendarURL.replace('webcal', 'https'));
		});
	...
	describe('GET /portal/day-rotation', function () {
		...
		it('gets all the day rotations', async function () {
			...
			for (const [year, months] of Object.entries(
				res.body.data.days as GetPortalDayRotationResponse['days']
			)) {
